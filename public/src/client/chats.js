...
define('forum/chats', [
	...
	Chats.addEventListeners = function () {
		...
		Chats.addCharactersLeftHandler($('[data-component="chat/main-wrapper"]'));
		Chats.addIPHandler($('[data-component="chat/main-wrapper"]'));
		Chats.createAutoComplete($('[data-component="chat/input"]'));
		...
		el.off('scroll').on('scroll', function () {
			...
			socket.emit('modules.chats.getMessages', {
				...
				data = data.filter(function (chatMsg) {
					return !$('[data-component="chat/message"][data-mid="' + chatMsg.messageId + '"]').length;
				});
				if (!data.length) {
					loading = false;
					return;
	...
	Chats.addMemberHandler = function (roomId, buttonEl) {
		...
		buttonEl.on('click', function () {
			Benchpress.parse('modals/manage-room', {}, function (html) {
	...
	Chats.refreshParticipantsList = function (roomId, modal) {
		socket.emit('modules.chats.getUsersInRoom', { roomId: roomId }, function (err, users) {
			...
			app.parseAndTranslate('modals/manage-room-users', {
				users: users,
			}, function (html) {
				listEl.html(html);
			});
	...
	Chats.addRenameHandler = function (roomId, buttonEl, roomName) {
		...
		buttonEl.on('click', function () {
			Benchpress.parse('modals/rename-room', {
	...
	Chats.addSocketListeners = function () {
		...
		socket.on('event:user_status_change', function (data) {
			app.updateUserStatus($('.chats-list [data-uid="' + data.uid + '"] [data-component="user/status"]'), data.status);
		});
		...
		socket.on('event:chats.roomRename', function (data) {
			...
			var titleEl = roomEl.find('[data-component="chat/title"]');
	...
	Chats.setActive = function () {
		if (ajaxify.data.roomId) {
			...
			$('.expanded-chat [data-component="chat/input"]').focus();
		}
