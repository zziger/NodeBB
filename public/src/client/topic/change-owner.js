define('forum/topic/change-owner', [
	...
	ChangeOwner.init = function (postEl) {
		...
		app.parseAndTranslate('modals/change-post-owner', {}, function (html) {
			modal = html;
	...
	function showPostsSelected() {
		if (postSelect.pids.length) {
			modal.find('#pids').translateHtml('[[topic:selected-post-count, ' + postSelect.pids.length + ']]');
		} else {
			modal.find('#pids').translateHtml('[[topic:no-posts-selected]]');
		}
	}
